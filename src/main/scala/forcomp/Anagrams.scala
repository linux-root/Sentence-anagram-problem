...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy(identity).map(entry => entry._1 -> entry._2.size).toList.sorted
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = if (s.isEmpty) List.empty else wordOccurrences(s.reduce((w1, w2) => w1.concat(w2)))
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    Dictionary.loadDictionary.groupBy(wordOccurrences) withDefaultValue (List())
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for ((char, frequency) <- x if frequency - mapY.getOrElse(char, 0) > 0) yield (char, frequency - mapY.getOrElse(char, 0))
  }
